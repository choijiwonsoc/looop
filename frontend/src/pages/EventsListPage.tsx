...
export function EventsListPage() {
  ...
  const filtered = typeFilter
    ? enriched.filter((e) => e.event.type === typeFilter)
    : enriched;
  ...
  if (loading) {
    return (
      <div className="max-w-6xl mx-auto px-6 sm:px-10 py-16 text-center text-ink-soft">
        Loading events…
      </div>
    );
  }
  ...
  if (loadError) {
    return (
      <div className="max-w-6xl mx-auto px-6 sm:px-10 py-16 text-center">
        ...
        <button onClick={loadEvents} className="text-loop text-sm font-medium">
          Try again
        </button>
      </div>
    );
    ...
    <div className="max-w-6xl mx-auto px-6 sm:px-10 py-8 sm:py-10">
      ...
      <div className="flex items-stretch bg-white border border-line rounded-xl overflow-hidden mb-6">
        {[
          { value: events.length, label: "Total events", tone: "text-ink" },
          {
            ...
            tone: needsAttention > 0 ? "text-normal" : "text-optional",
          },
          {
            value: totalUrgent,
            label: "Urgent tasks open",
            tone: totalUrgent > 0 ? "text-urgent" : "text-optional",
          },
          {
            value: totalIssues,
            label: "Issues open",
            tone: totalIssues > 0 ? "text-urgent" : "text-optional",
          },
        ].map((s, i) => (
          <div
            key={s.label}
            className={`flex-1 px-4 py-3 ${i > 0 ? "border-l border-line" : ""}`}
          >
            <div className={`text-xl ${s.tone}`}>{s.value}</div>
            <div className="text-[11px] uppercase tracking-wide text-ink-soft mt-0.5">
              {s.label}
            </div>
          </div>
        ))}
      ...
      {sorted.length === 0 ? (
        <div className="text-center py-16">
          ...
          <Link to="/new" className="text-loop font-medium text-sm">
